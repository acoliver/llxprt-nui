...
export function findTheme(themes: ThemeDefinition[], key: string): ThemeDefinition | undefined {
  const normalized = key.trim().toLowerCase();
  return themes.find((theme) => theme.slug.toLowerCase() === normalized || theme.name.toLowerCase() === normalized);
}
