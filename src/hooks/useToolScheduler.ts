...
type OnCompleteCallback = (completedTools: CompletedToolCall[]) => Promise<void> | void;
...
function applyOutputUpdate(
  ...
  return prevCalls.map((call) => updateToolCallOutput(call, toolCallId, outputChunk));
}
...
function transformToTrackedCalls(
  ...
  return updatedCalls.map((call) => ({
    ...
    responseSubmittedToModel:
      previousCallMap.get(call.request.callId)?.responseSubmittedToModel ?? false,
  })) as TrackedToolCall[];
}
...
function getToolCallDescription(call: TrackedToolCall): string {
  if ("invocation" in call) {
    const invocation = call.invocation as { getDescription(): string } | undefined;
    if (invocation) {
...
function getToolCallOutput(call: TrackedToolCall): string | undefined {
  if (call.status === "success" || call.status === "error" || call.status === "cancelled") {
...
function markCallsAsSubmitted(
  ...
  const hasMatch = prevCalls.some((call) => callIdsToMark.includes(call.request.callId));
  if (!hasMatch) {
    return prevCalls;
  }
...
export function useToolScheduler(
  ...
  useEffect(() => {
    ...
    const handleOutputUpdate = (toolCallId: string, outputChunk: string): void => {
    ...
    const handleAllComplete = async (completedToolCalls: CompletedToolCall[]): Promise<void> => {
      logger.debug("handleAllComplete called", "toolCount:", completedToolCalls.length);
      if (completedToolCalls.length > 0) {
        ...
        logger.debug("handleAllComplete: no completed tools, skipping callback");
      }
  ...
  const respondToConfirmation: RespondToConfirmationFn = useCallback(
    (callId: string, outcome: ToolConfirmationOutcome) => {
      logger.debug("respondToConfirmation called", "callId:", callId, "outcome:", outcome);
      ...
      if (!toolCall) {
        logger.warn("respondToConfirmation: tool call not found or not awaiting approval", "callId:", callId);
        return;
      }
      ...
      if (waitingCall.confirmationDetails?.onConfirm) {
        ...
        logger.warn("respondToConfirmation: no onConfirm callback found", "callId:", callId);
      }
    },
    [toolCalls]
  );
