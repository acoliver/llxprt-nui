...
const APPROVAL_OPTIONS: ToolApprovalOutcome[] = ["allow_once", "allow_always", "cancel"];
 ...
 */
export function useApprovalKeyboard(options: UseApprovalKeyboardOptions): UseApprovalKeyboardResult {
  ...
  useEffect(() => { canAllowAlwaysRef.current = canAllowAlways; }, [canAllowAlways]);
  ...
  useKeyboard((key) => {
    ...
    switch (key.name) {
      case "up":
        setSelectedIndex((prev) => (prev > 0 ? prev - 1 : currentOptionCount - 1));
        handled = true;
        break;
      case "down":
        setSelectedIndex((prev) => (prev < currentOptionCount - 1 ? prev + 1 : 0));
        handled = true;
        break;
      case "return":
      case "kpenter": {
        const outcome = currentOptions[selectedIndexRef.current];
        logger.debug("Enter pressed", "selectedIndex:", selectedIndexRef.current, "outcome:", outcome);
        if (outcome) {
