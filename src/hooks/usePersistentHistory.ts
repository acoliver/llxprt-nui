...
import { PersistentHistoryService, createPersistentHistory } from "../features/chat/persistentHistory";
 ...
 */
export function usePersistentHistory(options: UsePersistentHistoryOptions): UsePersistentHistoryReturn {
  ...
  useEffect(() => {
    ...
    const initService = async () => {
      logger.debug("Initializing persistent history", { workingDir, sessionId });
      ...
      try {
        ...
        setIsReady(true);
        logger.debug("Persistent history ready", { historyCount: newService.count });
      } catch (err) {
