...
import type { ConfigSession, ConfigSessionOptions } from "../features/config/configSession";
...
export function useSessionManager(): UseSessionManagerReturn {
  ...
  const createSession = useCallback(
    async (options: ConfigSessionOptions): Promise<void> => {
      logger.debug("Creating new session", { model: options.model, provider: options.provider });
