...
import type { GeminiClient, ConfigParameters } from "@vybestack/llxprt-code-core";
...
function requiresProviderManager(providerName: string | undefined): boolean {
  ...
  const name = providerName.toLowerCase();
  return name === "openai" || name === "openai-responses" || name === "anthropic";
}

export function createConfigSession(options: ConfigSessionOptions): ConfigSession {
  ...
  return {
    ...
    getClient(): GeminiClient {
      if (!client) {
        throw new Error("ConfigSession not initialized. Call initialize() first.");
      }
      return client;
    },
