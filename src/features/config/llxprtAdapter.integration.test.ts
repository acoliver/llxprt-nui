...
function loadSyntheticProfile(): ProfileData | null {
  ...
  try {
    return JSON.parse(readFileSync(SYNTHETIC_PROFILE_PATH, "utf8")) as ProfileData;
  } catch {
    return null;
  }
}
...
describe("llxprtAdapter end-to-end", () => {
  describeSynthetic("with synthetic profile", () => {
      ...
      async () => {
        ...
        try {
          for await (const event of sendMessageWithSession(
            ...
            const textDeltaCount = events.filter(e => e.type === "text_delta").length;
            if (textDeltaCount >= 3) {
              controller.abort();
              break;
            }
        ...
        const textDeltas = events.filter(
          (e): e is Extract<AdapterEvent, { type: "text_delta" }> => e.type === "text_delta"
        );
     ...
     */
    it(
      "initializes session successfully",
      async () => {
        const session = createSessionFromProfile(syntheticProfile!);
        expect(session).not.toBeNull();
     ...
     */
    it(
      "has tools registered after initialization",
      async () => {
        const session = createSessionFromProfile(syntheticProfile!);
        expect(session).not.toBeNull();
     ...
     */
    it(
      "uses correct model from profile",
      async () => {
        const session = createSessionFromProfile(syntheticProfile!);
        expect(session).not.toBeNull();
