...
const SLASH_COMMANDS: SlashNode[] = [
  ...
  { name: "docs", description: "open full LLxprt Code documentation in your browser" },
  ...
  { name: "quit", description: "exit the cli" },
  {
    ...
    children: [
      ...
      { name: "cache", description: "Show cache usage statistics (Anthropic only)." }
    ]
  },
    ...
    children: [
      ...
      { name: "modelparam", description: "Model parameter option" },
      {
        ...
        children: [
          ...
          { name: "auto", description: "Automatically filter inappropriate emojis" },
      ...
      { name: "compression-threshold", description: "Compression Threshold option" },
...
export function getSlashSuggestions(parts: string[], limit: number): SlashSuggestion[] {
...
export function extractSlashContext(input: string, cursorOffset: number): { parts: string[]; start: number; end: number } | null {
