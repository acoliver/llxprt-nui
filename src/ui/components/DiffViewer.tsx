...
function parseDiffLine(line: string, state: ParsedDiffState): { diffLine: DiffLine | null; newState: ParsedDiffState } {
  const hunk = parseHunkHeader(line);
  if (hunk) {
    return {
      diffLine: { type: "hunk", content: line },
      newState: { currentOldLine: hunk.oldStart, currentNewLine: hunk.newStart, inHunk: true },
    };
  }
  ...
  if (line.startsWith(" ")) {
    ...
    const newLine = state.currentNewLine + 1;
    return {
      diffLine: { type: "context", oldLine, newLine, content: line.substring(1) },
...
function parseDiff(diffContent: string): DiffLine[] {
  ...
  let state: ParsedDiffState = { currentOldLine: 0, currentNewLine: 0, inHunk: false };
...
function DiffLineRow({ line, index, gutterWidth, colors }: DiffLineProps): JSX.Element {
  ...
  const textProps = lineBg !== undefined
    ? { fg: lineFg, bg: lineBg }
    : { fg: lineFg };

  return (
    <box key={`diff-line-${index}`} flexDirection="row" style={{ width: "100%" }}>
  ...
  const content = (
    <box flexDirection="column" style={{ gap: 0, width: "100%" }}>
      ...
      {displayableLines.map((line, index) => (
        <DiffLineRow key={`line-${index}`} line={line} index={index} gutterWidth={gutterWidth} colors={colors} />
      ))}
    </box>
  );
