...
export function RadioSelect<T>(props: RadioSelectProps<T>): JSX.Element {
  const { options, onSelect, theme, isFocused = true, initialIndex = 0 } = props;
  const { selectedIndex, moveSelection, setSelectedIndex } = useListNavigation(options.length);

  // Initialize to initialIndex on first render
  if (selectedIndex === 0 && initialIndex !== 0 && initialIndex < options.length) {
