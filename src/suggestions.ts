...
export function getSuggestions(query: string, limit: number = MAX_SUGGESTIONS): string[] {
  ...
  const matches = FILE_ENTRIES.filter((entry) => {
    ...
    return segments.some((segment) => segment.toLowerCase().startsWith(normalized));
  });
...
export function extractMentionQuery(input: string, cursorOffset: number): string | null {
