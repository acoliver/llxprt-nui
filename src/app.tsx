...
import type { CompletedToolCall, WaitingToolCall, ToolCallConfirmationDetails } from "@vybestack/llxprt-code-core";
...
import { useToolScheduler, type TrackedToolCall, type ScheduleFn } from "./hooks/useToolScheduler";
...
import { ChatLayout, type PendingApprovalState, type ToolApprovalOutcome } from "./ui/components/ChatLayout";
...
function getQuestionForConfirmation(details: ToolCallConfirmationDetails): string {
  switch (details.type) {
    case "edit":
...
function getPreviewForConfirmation(details: ToolCallConfirmationDetails): string {
  switch (details.type) {
    case "edit":
...
function AppInner(): JSX.Element {
  ...
  const abortRefContainer = useRef<{ current: AbortController | null }>({ current: null });
  ...
  const onToolsComplete = useCallback(async (completedTools: CompletedToolCall[]) => {
    logger.debug("onToolsComplete called", "toolCount:", completedTools.length);

    if (!session || completedTools.length === 0) {
      logger.debug("onToolsComplete: no session or empty tools, setting idle");
      setStreamState("idle");
      return;
    }
    ...
    if (continuationInProgressRef.current) {
      logger.debug("onToolsComplete: skipping, continuation already in progress");
      return;
    }
    ...
    logger.debug("onToolsComplete: checking signal", "hasSignal:", !!signal, "aborted:", signal?.aborted, "hasScheduler:", !!scheduleFn);
    ...
    if (signal && !signal.aborted && scheduleFn) {
      ...
      try {
        ...
        logger.debug("onToolsComplete: continueStreamingAfterTools finished", "hasMoreTools:", hasMoreTools);
      } finally {
        continuationInProgressRef.current = false;
      }
    } else {
      logger.debug("onToolsComplete: signal aborted or no scheduler, setting idle");
      setStreamState("idle");
    }
  }, [session, appendMessage, appendToMessage, appendToolCall, updateToolCall, setResponderWordCount, setStreamState]);

  const onToolCallsUpdate = useCallback((tools: TrackedToolCall[]) => {
    for (const tool of tools) {
      ...
      switch (tool.status) {
        ...
        case "success": {
          ...
          const update: { status: "complete"; output?: string } = { status: "complete" };
          if (completed.response.resultDisplay != null) {
            update.output = typeof completed.response.resultDisplay === "string"
        ...
        case "error": {
          ...
          const errorUpdate: { status: "error"; errorMessage?: string } = { status: "error" };
  ...
  const { pendingApproval, queueApprovalFromScheduler, handleDecision, clearApproval } = useToolApproval(respondToConfirmation);
  ...
  const { mountedRef, abortRef, cancelStreaming, startStreamingResponder } = useStreamingLifecycle(
    appendMessage,
    appendToMessage,
    appendToolCall,
    updateToolCall,
  ...
  const handleThemeSelect = useCallback((t: ThemeDefinition) => { setThemeBySlug(t.slug); }, [setThemeBySlug]);
  ...
  const handleApprovalSelectKeyboard = useCallback((outcome: ToolApprovalOutcome) => {
    const current = pendingApprovalRef.current;
    logger.debug("handleApprovalSelectKeyboard called", "outcome:", outcome, "callId:", current?.callId);
    if (current) {
  ...
  const handleApprovalSelectFromUI = useCallback((callId: string, outcome: ToolApprovalOutcome) => {
  ...
  useEnterSubmit(() => void handleSubmit(), dialog.isOpen || pendingApproval !== null);
  useSuggestionKeybindings(
    dialog.isOpen || pendingApproval !== null ? 0 : suggestions.length,
    moveSelection,
    handleTabComplete,
    handleCancelAll,
    () => { textareaRef.current?.clear(); enforceInputLineBounds(); return Promise.resolve(); },
    () => streamState === "busy",
    () => (textareaRef.current?.plainText ?? "").trim() === ""
  );
  useHistoryNavigation(dialog.isOpen || pendingApproval !== null, suggestions.length, handleHistoryKey);
  ...
  return (
    <>
      ...
      <ChatLayout headerText={HEADER_TEXT} entries={entries} scrollRef={scrollRef} autoFollow={autoFollow}
        ...
        {...(pendingApprovalState ? { pendingApproval: pendingApprovalState } : {})}
        {...(pendingApprovalState ? { onApprovalSelect: handleApprovalSelectFromUI } : {})} />
    </>
  );
}
