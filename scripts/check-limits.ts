...
function isFunctionWithBody(node: ts.Node): node is ts.FunctionLikeDeclarationBase & { body: ts.Block } {
  return ts.isFunctionLike(node) && node.body !== undefined && ts.isBlock(node.body as ts.Node);
}
