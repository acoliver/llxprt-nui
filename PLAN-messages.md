...
**ThinkingMessage:**
```tsx
<text fg={theme.colors.text.thinking} style={{ fontStyle: "italic" }}>{text}</text>
```
...
describe("roleColor", () => {
  ...
  it("should return systemText color for system role", () => {
    expect(roleColor("system", mockTheme)).toBe(mockTheme.colors.message.systemText);
  });

  it("should return thinking text color for thinking role", () => {
    expect(roleColor("thinking", mockTheme)).toBe(mockTheme.colors.text.thinking);
  });
});
```
